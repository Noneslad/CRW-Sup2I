...
class WebTools {
    ...
    public static function checkField($term,$redirectView){
        if(self::fromRequest($term)){
            return true;
        }elseIf(self::fromFileRequest()){
            return true;
        }
        else{
            self::setInFlashBag('Danger','Le champs : '.ucfirst($term)).' est requis';
            self::redirect($redirectView);
        }
    }
    ...
    public static function fromFileRequest(){
